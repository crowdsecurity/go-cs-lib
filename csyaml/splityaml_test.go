...
func TestSplitDocumentsDecEnc(t *testing.T) {
	tests := []struct {
		name    string
		input   string
		want    [][]byte
...
`,
		...
		},
		{
			...
			input:   "list: [1, 2,",
			want:    nil,
			wantErr: "decode doc 0: yaml: line 1: did not find expected node content",
		},
		{
...
`,
			want: [][]byte{
				[]byte("# comment 1\na: 1\n# comment 2\nb: 2\n"),
				// not sure how to get rid of the extra new line here.
				[]byte("# comment 3\n- 1\n# comment 4\n- 2\n\n# comment 5\n"),
				[]byte("# comment 6\n\"scalar\"\n# comment 7\n"),
			},
		},
...
`,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			docs, err := csyaml.SplitDocumentsDecEnc(r)
